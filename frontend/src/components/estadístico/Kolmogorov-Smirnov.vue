<template>
  <div class="grafico-container">
    <h1>Prueba de Normalidad Kolmogorov-Smirnov</h1>
    ...
    <div v-if="resultado">
      <h3>Resultado de la Prueba Kolmogorov-Smirnov</h3>
      <p><strong>Estadístico KS:</strong> {{ resultado.ks.toFixed(4) }}</p>
  ...
  <canvas id="grafico-ks" width="600" height="300" style="display: none;"></canvas>
</template>
  ...
  methods: {
    ...
    async ejecutarPrueba() {
      ...
      if (this.tipoSeleccionado === "Crecimiento") {
        ...
        registros = crecimientos.data.filter(
          d => d.siembra === this.siembraSeleccionada && d.rubro === rubro.id_rubro
        );
        this.datos = registros.map(r => r.medicion);
        this.fechas = registros.map(r => r.fecha);
      } else {
        const map = {
        ...
        registros = res.data.filter(d => d.siembra === this.siembraSeleccionada && d[field] !== null);
        this.datos = registros.map(d => d[field]);
        this.fechas = registros.map(d => d.fecha);
      }

      const media = this.media(this.datos);
      const desviacion = this.desviacion(this.datos);
      const emp = this.datos.slice().sort((a, b) => a - b);
      const n = emp.length;

      let maxDif = 0;
      for (let i = 0; i < n; i++) {
        const F = (i + 1) / n;
        const N = normal.cdf(emp[i], media, desviacion);
        maxDif = Math.max(maxDif, Math.abs(F - N));
      }

      const D = maxDif;
      const p = Math.exp(-2 * D * D * n);

      this.resultado = { ks: D, p, media, desviacion, n, emp };
    },
    media(arr) {
    ...
    exportarExcel() {
      ...
      const rows = this.datos.map((val, i) => [i + 1, this.formatDate(this.fechas[i]), val]);
      const wb = XLSX.utils.book_new();
      const ws = XLSX.utils.aoa_to_sheet([...headers, ...rows]);
      XLSX.utils.book_append_sheet(wb, ws, "Datos");
      const nombre = `ks_datos_${this.tipoSeleccionado}_${this.campoSeleccionado.replace(/\s+/g, "_")}_${Date.now()}.xlsx`;
    ...
    async generarPDF() {
      ...
      doc.setFontSize(16);
      doc.text("Prueba de Normalidad Kolmogorov-Smirnov", 14, 20);
      doc.setFontSize(12);
      const info = [
        ...
        `Media: ${this.resultado.media.toFixed(2)}`,
        `Desviación Est.: ${this.resultado.desviacion.toFixed(2)}`,
        `Estadístico KS: ${this.resultado.ks.toFixed(4)}`,
        `Valor p: ${this.resultado.p.toFixed(4)}`,
        `Conclusión: ${this.resultado.p < 0.05 ? "Se rechaza H0 - No normal" : "No se rechaza H0 - Normal"}`
      ];
      info.forEach((linea, i) => doc.text(linea, 14, 30 + i * 7));

      const data = this.datos.map((val, i) => [i + 1, this.formatDate(this.fechas[i]), val]);
      autoTable(doc, {
        startY: 30 + info.length * 7 + 10,
        head: [["#", "Fecha", "Valor"]],
        body: data,
        styles: { fontSize: 10, cellPadding: 3, halign: "center" },
        columnStyles: { 1: { cellWidth: 30, textDirection: 90 } },
        headStyles: {
          fillColor: [40, 167, 69],
          textColor: 255,
          fontStyle: 'bold'
        ...
        didDrawPage: () => {
          ...
          doc.text(`Módulo: Kolmogorov-Smirnov | Generado: ${fechaGeneracion}`, 14, pageHeight - 15);
        },
        pageBreak: 'auto'
      });

      await this.generarGraficoPDF("bar");
      const canvas = document.getElementById("grafico-ks");
      ...
      const nombre = `ks_test_${tipo}_${campo.replace(/\s+/g, "_")}_${Date.now()}.pdf`;
      doc.save(nombre);
    },
    async generarGraficoPDF(tipo = "bar") {
      return new Promise((resolve) => {
        const ctx = document.getElementById("grafico-ks").getContext("2d");
        if (this.chartInstance) this.chartInstance.destroy();
        this.chartInstance = new Chart(ctx, {
          type: tipo,
          data: {
            labels: this.fechas.map(f => this.formatDate(f)),
            datasets: [{
              label: "Valores",
              data: this.datos,
