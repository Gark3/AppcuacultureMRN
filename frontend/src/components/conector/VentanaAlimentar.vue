<template>
  <!-- Tarjeta de encabezado + buscador -->
  <section class="card">
    <div class="card__header">
      <h2 class="card__title">Siembras Activas</h2>
      <p class="card__sub">Filtra y registra la <strong>alimentación</strong> de tus siembras activas.</p>
    </div>
          ...
          >
            ...
            <div class="mt-2">
              <button
                ...
                @click="registrarAlimentacion(s.id_siembra)"
...
export default {
  name: 'SiembrasActivasAlimentacion',
  data() {
    return {
      siembras: [],
      busqueda: '',
  ...
  computed: {
    ...
    pages() { return Math.max(1, this.total - this.visible + 1); }, // desplazar 1 por vez
  ...
  methods: {
    ...
    registrarAlimentacion(id) {
      this.$router.push(`/producción/alimentar/registro/${id}`);
    },
